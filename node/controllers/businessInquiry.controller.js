...
const BusinessInquiryModal = require('../schemas/businessInquiry.schema')

const BusinessInquiryController = asyncHandler(async (req, res) => {
    const { fullName, businessEmail, message, subject, countryCode, phone } = req.body;
    if (!fullName || !businessEmail || !subject || !message || !countryCode || !phone) {
                                             ...
                                             <tbody style="border:none;">
                                                 <tr style="border:none;">
                                                     <td style="border:none;font-family: 'Open Sans', Arial, sans-serif; font-size:20px; font-weight: bold; color:#000244;text-align: center;">
                                                         Business Inquiry Form Submission
                                                     </td>
                                                 </tr>
                                                 <tr style="border:none;">
    ...
    try {
        ...
        const mailOptions = {
            ...
            subject: `Velosting | Business Inquiry Form`,
            html: emailTemp
        };
        const status = await transporter.sendMail(mailOptions)
        if (status) {
            const newContact = await BusinessInquiryModal.create({
                fullName: fullName,
...
module.exports = { BusinessInquiryController }