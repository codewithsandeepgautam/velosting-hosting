...
const InquiryForm = () => {
    ...
    function handleSubmitData(e) {
        e.preventDefault();
        setError((prevState) => ({ ...prevState, message: "" }));
        if (!formData.name || !formData.email || !formData.phone || !formData.message || !formData.countryCode) {
            setError((prevState) => ({
                ...prevState,
                success: false,
            ...
            setLoading(true);
            PostRequest(`${process.env.REACT_APP_URL}/inquiry`, {
                name: formData.name,
                            ...
                            <form onSubmit={handleSubmitData}>
                                ...
                                <div className='grid md:grid-cols-2 md:gap-4'>
                                    <div className='relative z-0 w-full md:mb-[25px] max-md:mb-[15px] group'>
                                        ...
                                        <input type="text" name='name' className='md:h-[50px] max-md:h-[44px] w-full focus:outline-none border-[#ddd] px-[8px] border focus:border-[#00A51A]/[40%] rounded-[5px] text-[16px]' value={formData.name} onChange={fullNameHandler} />
                                    </div>
                                    <div className='relative z-0 w-full md:mb-[25px] max-md:mb-[15px] group'>
                                        <label htmlFor="email" className="block mb-[5px] text-[16px]">Email Address<span className="text-[#FF0000]"> *</span></label>
                                        <input type="email" name='email' className='md:h-[50px] max-md:h-[44px] w-full focus:outline-none border-[#ddd] px-[8px] border focus:border-[#00A51A]/[40%] rounded-[5px] text-[16px]' placeholder='example@gmail.com' value={formData.email} onChange={handleEmailInputChange} />
                                    </div>
                                </div>
                                <div className='relative z-0 w-full md:mb-[25px] max-md:mb-[15px] group'>
                                    <label htmlFor="phone" className="block mb-[5px] text-[16px]">Phone<span className="text-[#FF0000]"> *</span></label>
                                    <div className='flex justify-between'>
                                        <select className="md:h-[50px] max-md:h-[44px] px-[6px] w-[130px] !border-r-0 border border-[#ddd] rounded-l-[5px] focus:outline-none" value={formData?.countryCode || '+91'}  onChange={(e) => {
                                ...
                                <div className='relative z-0 w-full md:mb-[25px] max-md:mb-[15px] group'>
                                    ...
                                    <textarea id="message" className="md:h-[100px] max-md:h-[90px] w-full focus:outline-none border-[#ddd] p-[8px] border focus:border-[#00A51A]/[40%] rounded-[5px] text-[16px]" value={formData.message} onChange={handleMessageInputChange}></textarea>
                                </div>
